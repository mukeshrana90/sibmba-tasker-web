  ...
  const handleAccept = (data, status) => {
    ...
    dispatch(CorporateActions.acceptRejectCorporateSuggestion(payload))
      .then((res) => {
        if (res?.payload) {
          toast.success(
            status === 1 ? "Accepted successfully." : "Rejected successfully."
          );
                               ...
                               <ul className="list-unstyled">
                                {upcomingTasks?.length > 0 ? (
                                  upcomingTasks.map((res, idx) => {
                                    return res?.type === 'task' ? (
                                      <li key={idx} className="mb-3">
                                        <div className="booking-card">
                                          <div className="d-flex justify-content-between">
                                            <h5 className="mb-2">{res?.taskId?.need_done}</h5>
                                            <h5 className="mb-2 corporate_inner pending">Pending</h5>
                                          </div>
                                          <p className="mb-1 small text-muted">{res?.taskId?.details}</p>
                                          <p className="mb-1 mt-2">Address: {res?.taskId?.address}</p>
                                          <div className="d-flex gap-3 mt-3">
                                            <small className="text-muted">Time: {res.taskId?.task_time}</small>
                                            <small className="text-muted">Date: {res.taskId?.when_done}</small>
                                          </div>
                                        </div>
                                      </li>
                                    ) : (
                                      <li key={idx}>
                                        <div className="booking-card">
                                          <div className="d-flex justify-content-between">
                                            <h5 className="mb-2">{res?.userId?.full_name}</h5>
                                            <h5 className="mb-2 corporate_inner pending">Pending</h5>
                                          </div>
                                          <p className="text-muted mb-0">{res?.userId?.email}</p>
                                          <p className="mb-1 mt-2">Address: {res?.bookingId?.address}</p>
                                          <div className="d-flex gap-3 mt-3">
                                            <small className="text-muted">Time: {res.bookingId?.slotTime[idx]}</small>
                                            <small className="text-muted">Date: {res.bookingId?.date}</small>
                                          </div>
                                        </div>
                                      </li>
                                    );
                                  ...
                                  <div className="no-upcoming-bookings">
                                    {/* Your existing SVG and no-leads message */}
                                    <svg
                                            ...
                                            status === "rejected" ? (
                                              <div className="book-service-action-btn leads-btn d-flex gap-2">
                                                <button
                                                  ...
                                                  onClick={() =>
                                                    navigate(
                                                      `/corporate/lead-details/${res?.bookingId?._id || res?.taskId?._id}?type=${res.type}`
                                                    )
                                                  }
                                                >
                                                  See More
