...
import { SimbaFeedbackModalBody, SimbaFeedbackThankYou } from "../CommanComponents/BookingDetail/SimbaBookingDetailParts";
...
export default function UserBookingDetails() {
  ...
  const renderModalContent = () => {
    if (showFeedback) {
      const thumbSrc =
        bookingState?.serviceSubCategory?.images?.length > 0
          ? serviceImageUrl(bookingState.serviceSubCategory.images[0])
      ...
      const title =
        ...
        task?.address ||
        "";
      const scheduleText = `${bookingState?.slotTime || task?.task_time || "—"}, ${moment(
        bookingState?.date || task?.when_done || undefined
      ).format("DD MMM")}`;
      const locationText = bookingState?.address || task?.address || "—";

      return (
        <SimbaFeedbackModalBody
          thumbSrc={thumbSrc}
          title={title}
    ...
    if (showThankYou) {
      return <SimbaFeedbackThankYou onDone={handleThankYouClose} />;
    }
    return null;
  };
      ...
      <div className="card">
        ...
        <div className="d-flex align-items-center justify-content-between gap-2 mb-2">
          ...
          <small style={{ color: "var(--muted)", fontWeight: 600 }}>
            {moment(existingFeedback?.createdAt || new Date()).format("DD MMM")}
          </small>
        </div>
  ...
  const shareMapAt = async (lat, lng) => {
    if (lat == null || lng == null) return;
    const shared = await tryNativeLocationShare(routeShareUrl);
    if (!shared) {
      setShareLocationUrl(routeShareUrl);
  ...
  return (
    <Layout footerVariant="marketing">
      <UserBookingDetailMain
        task={task}
        bookingState={bookingState}
        taskbooking={taskbooking}
        selectedQuotation={selectedQuotation}
        posterTaskStepperStatus={posterTaskStepperStatus}
        isReschedulePendingForUser={isReschedulePendingForUser}
        bookingShowJobDoneBtn={bookingShowJobDoneBtn}
        bookingShowPayBtn={bookingShowPayBtn}
        taskShowJobDoneBtn={taskShowJobDoneBtn}
        taskShowPayBtn={taskShowPayBtn}
        jobDoneSubmitting={jobDoneSubmitting}
        rescheduleAcceptSubmitting={rescheduleAcceptSubmitting}
        canRaiseBookingDispute={canRaiseBookingDispute}
        canRaiseTaskDispute={canRaiseTaskDispute}
        hasSubmittedFeedback={hasSubmittedFeedback}
        bookingDisputes={bookingDisputes}
        visibleBookingDisputes={visibleBookingDisputes}
        showAllBookingDisputes={showAllBookingDisputes}
        setShowAllBookingDisputes={setShowAllBookingDisputes}
        taskDisputes={taskDisputes}
        visibleTaskDisputes={visibleTaskDisputes}
        showAllTaskDisputes={showAllTaskDisputes}
        setShowAllTaskDisputes={setShowAllTaskDisputes}
        getBookingDisputeRaisedByName={getBookingDisputeRaisedByName}
        getBookingDisputeRaisedByClass={getBookingDisputeRaisedByClass}
        getTaskDisputeRaisedByName={getTaskDisputeRaisedByName}
        getTaskDisputeRaisedByClass={getTaskDisputeRaisedByClass}
        corporateSuggestions={corporateSuggestions}
        selectedCorporateIds={selectedCorporateIds}
        toggleSelect={toggleSelect}
        shouldShowOnWayMap={shouldShowOnWayMap}
        routeEmbedUrl={routeEmbedUrl}
        mapLat={mapLat}
        mapLng={mapLng}
        openMapAt={openMapAt}
        shareMapAt={shareMapAt}
        renderExistingFeedback={renderExistingFeedback}
        navigate={navigate}
        handleEditOpen={handleEditOpen}
        setSelectedBoooking={setSelectedBoooking}
        handleAcceptRescheduledBooking={handleAcceptRescheduledBooking}
        openJobDoneConfirmModal={openJobDoneConfirmModal}
        ...
        handleAcceptCrop={handleAcceptCrop}
        setCorporateProfile={setCorporateProfile}
      />
      ...
      <Modal
        ...
        centered
        dialogClassName="simba-feedback-dialog"
        contentClassName="simba-feedback-content border-0 shadow-none"
      >
        <div className="simba-feedback-panel">
          {showFeedback && (
      ...
      <ShareLocationModal
        show={showShareLocationModal}
        onHide={() => setShowShareLocationModal(false)}
