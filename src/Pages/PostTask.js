...
const validationSchema = Yup.object({
  ...
  // images: Yup.mixed()
  //   .test("fileCount", "Minimum 1 image required", (value) => {
  //     return value && value.length >= 1;
  //   })
  //   .test("fileCount", "Maximum 6 images allowed", (value) => {
...
export default function PostTask() {
  ...
  const serviceImagesInputRefs = useRef(
    Array(6).fill().map(() => React.createRef())
  );
  ...
  const handleFileChange = (event, setFieldValue, values) => {
    ...
    const validFiles = Array.from(files).filter((file) => file.size <= 10 * 1024 * 1024); // 10 MB limit
            ...
            <Formik
              ...
              onSubmit={async (values, { setSubmitting }) => {
                ...
                try {
                  ...
                  const action = CustomerActions.createPost(formData);
                  ...
                  if (response?.payload?.status_code === 200) {
                    ...
                    toast.error(response?.payload?.message || "Failed to create task");
                  }
                } catch (error) {
